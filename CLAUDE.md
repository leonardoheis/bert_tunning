...
**Known limitation, found at verification time — TF-IDF did not catch the cross-jurisdiction case it was built for.** Testing against a small real-document sample (`samples/`, BETO v2) confirmed the signal works correctly end-to-end and adds real value for at least one case (a payment-receipt document scores `tfidf_cosine_z=3.28` against a `2.52` threshold, correctly reinforcing what Mahalanobis already flagged) — but it did **not** fire for either of the two known cross-jurisdiction decrees in that sample (a Córdoba decree scored `0.469`, a Santa Fe one scored `-0.835`, both far below threshold), which is the exact failure mode this signal was motivated by. The likely explanation: a full legal decree shares hundreds of boilerplate legal-vocabulary tokens with in-distribution training documents, and TF-IDF's cosine distance is computed over the whole vector — one or two distinguishing tokens (a city name mentioned once or twice) get diluted by the shared boilerplate rather than dominating the distance. This is a genuine, currently-unresolved gap, not a bug: the signal is correctly implemented, tested, and calibrated, but doesn't solve the specific problem that motivated it. A targeted follow-up (e.g. a narrower keyword/entity check for known municipality names, discussed and explicitly not yet built) remains the more direct fix for that particular failure mode; TF-IDF cosine stays in place as a fourth, independent signal that may still catch other kinds of lexical drift (as the payment-receipt case shows) even though it underperformed on its original motivating case.
