...
def test_compute_svm_classifiers_cmd_help() -> None:
    result = CliRunner().invoke(compute_svm_classifiers_cmd, ["--help"])
    assert result.exit_code == 0
    assert "svm" in result.output.lower() or "retraining" in result.output.lower()


def test_compute_svm_classifiers_cmd_fails_on_class_mismatch(tmp_path: Path) -> None:
