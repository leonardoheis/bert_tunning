...
**Commit/push ownership:** applying file changes and running `uv run poe check` is fine for Codex to do on its own within a task. Committing (`git commit`), pushing (`git push`), and opening/editing a PR (`gh pr create`/`gh pr edit`) are the human's actions by default — Codex should report the change is ready and stop, only performing those git operations when explicitly asked to for that specific instance.

**Review every generated PR:** whenever a PR is created (by Codex when explicitly asked, or by the human), dispatch an agent to evaluate it using the repo's code-review skill and post comments on the PR only if there's something worth flagging — don't post a comment just to say "looks fine."
